    ...
    <x-nav sticky>
        <x-slot:brand>
            <a href="{{ route('home') }}" class="flex items-center gap-3" wire:navigate>
                <span class="flex size-10 items-center justify-center rounded-lg bg-green-700 text-white"
                    aria-hidden="true">
        ...
        <x-slot:actions>
            <div class="hidden items-center gap-1 lg:flex">
                <a href="#como-funciona" class="hover:bg-base-200 rounded-lg px-3 py-2 text-sm font-medium">Como
                    funciona</a>
                <a href="{{ route('reports.index') }}"
                    class="hover:bg-base-200 rounded-lg px-3 py-2 text-sm font-medium"
                    wire:navigate>Mapa/ocorrências</a>
                <a href="#para-municipios" class="hover:bg-base-200 rounded-lg px-3 py-2 text-sm font-medium">Para
                    municípios</a>
            </div>

            <x-theme-toggle class="btn btn-circle btn-ghost btn-sm" />

            <x-button label="Registrar ocorrência" icon="o-plus" :link="route('management.reports.create')"
            ...
            <x-dropdown class="btn-ghost btn-circle lg:hidden" icon="o-bars-3" right no-x-anchor>
                ...
                <x-menu-item title="Para municípios" icon="o-building-office-2" link="#para-municipios"
                    no-wire-navigate />
                <x-menu-separator />
        ...
        <div class="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
            <div class="flex flex-col items-center gap-6 text-center sm:flex-row sm:justify-between sm:text-left">
                <div class="flex items-center gap-3">
                    <span class="flex size-9 items-center justify-center rounded-lg bg-green-700 text-white"
                        aria-hidden="true">
                ...
                <nav class="flex flex-wrap justify-center gap-x-6 gap-y-2 text-sm">
                    ...
                    <a href="{{ route('management.reports.create') }}" class="hover:text-primary"
                        wire:navigate>Registrar ocorrência</a>
                    <a href="{{ route('management.dashboard') }}" class="hover:text-primary" wire:navigate>Painel de
                        gestão</a>
                </nav>
            </div>
