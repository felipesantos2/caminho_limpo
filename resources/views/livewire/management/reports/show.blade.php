<div class="mx-auto max-w-5xl">
    <x-header
        title="Detalhe do relato"
        subtitle="{{ $report->protocol }}"
        size="text-3xl"
        weight="font-bold"
        use-h1
        separator
    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(18rem,0.65fr)]">
        <x-card shadow class="border-base-content/10 overflow-hidden border">
            <x-slot:figure>
                <img
                    src="{{ $report->imageUrl() }}"
            ...
            <p class="text-base-content/80 mt-4 leading-8 whitespace-pre-line">{{ $report->description }}</p>
        </x-card>

        <div class="grid content-start gap-6">
            <x-card title="Acompanhamento" shadow class="border-base-content/10 border">
            ...
            <x-card title="Localização" shadow class="border-base-content/10 border">
                <p class="text-base-content/75 text-sm leading-6">{{ $report->address }}</p>

                @if ($report->latitude && $report->longitude)
                    <p class="text-base-content/60 mt-4 font-mono text-xs">
                        {{ $report->latitude }}, {{ $report->longitude }}
                    </p>
                @else
                    <p class="text-base-content/50 mt-4 text-xs">Coordenadas não informadas.</p>
                @endif
            </x-card>

            <x-button
                label="Voltar aos relatos"
                icon="o-arrow-left"
